...
digiasset_check_official() {
    ...
    if [ -f "$DGA_INSTALL_LOCATION/.officialdiginode" ]; then
        printf "%b Checking for Official DigiNode Install of DigiAsset Node: %bDETECTED%b\\n" "${TICK}" "${COL_LIGHT_GREEN}" "${COL_NC}"
        printf "\\n"
        is_dga_installed="yes"
    elif [ -d "$DGA_INSTALL_LOCATION" ]; then
        printf "%b Checking for Official DigiNode Install of DigiAsset Node: %bNOT DETECTED%b\\n" "${CROSS}" "${COL_LIGHT_RED}" "${COL_NC}"
        printf "\\n"
...
if [ "$DO_MAINNET_PORT_TEST" = "YES" ]; then
    ...
    if [ "$DGB_MAINNET_PORT_FWD_STATUS" = "OPEN" ] || [ "$DGB_MAINNET_PORT_FWD_STATUS" = "CLOSED" ]; then
        ...
        if [ "$DGB_MAINNET_PORT_FWD_STATUS" = "OPEN" ]; then
            ...
            printf "\\n" 
            printf "%b DigiByte $DGB_MAINNET_PORT_TEST_NETWORK Node found at address $DGB_MAINNET_PORT_TEST_ADDRESS:\\n" "${INDENT}"
            printf "\\n"
...
if [ "$DO_TESTNET_PORT_TEST" = "YES" ]; then
    ...
    if [ "$DGB_TESTNET_PORT_FWD_STATUS" = "OPEN" ] || [ "$DGB_TESTNET_PORT_FWD_STATUS" = "CLOSED" ]; then
        ...
        if [ "$DGB_TESTNET_PORT_FWD_STATUS" = "OPEN" ]; then
            ...
            printf "\\n" 
            printf "%b DigiByte $DGB_TESTNET_PORT_TEST_NETWORK Node found at address $DGB_TESTNET_PORT_TEST_ADDRESS:\\n" "${INDENT}"
            printf "\\n"
