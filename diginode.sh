...
startup() {
  ...
  import_diginode_settings   # Import or create diginode.settings file
  ...
  set_sys_variables          # Set the memory variables once we know we are on linux (these are stored in a function in the installer)
...
if [ $statusmonitor_last_run="" ]; then
    ...
    sed -i -e "/^externalip=/s|.*|externalip=\"$externalip\"|" $DGN_SETTINGS_FILE
    ...
    sed -i -e "/^internalip=/s|.*|internalip=\"$internalip\"|" $DGN_SETTINGS_FILE
    ...
    if [ $dga_status = "running" ]; then
      ...
      sed -i -e '/^dga_ver_local=/s|.*|dga_ver_local="$(date)"|' $DGN_SETTINGS_FILE
...
if [ $dga_status = "running" ] && [ $dams_first_run = ""  ]; then
    ...
    sed -i -e '/^dga_ver_local=/s|.*|dga_ver_local="$(date)"|' $DGN_SETTINGS_FILE
    ipfs_ver_local=$(ipfs version | cut -d ' ' -f 3)
    sed -i -e '/^ipfs_ver_local=/s|.*|ipfs_ver_local="$(date)"|' $DGN_SETTINGS_FILE
    dams_first_run=$(date)
